...
const push = (o: Pt[], x: number, y: number, z: number) => o.push([x, y, z]);
 ...
 */
function tube(o: Pt[], a: Pt, b: Pt, ra: number, rb: number, n: number, bulge = 0) {
  const ax = b[0] - a[0];
  const ay = b[1] - a[1];
  const az = b[2] - a[2];
...
function plane(o: Pt[], c: Pt, u: Pt, v: Pt, n: number) {
  for (let i = 0; i < n; i++) {
...
function seg(o: Pt[], a: Pt, b: Pt, n: number) {
  for (let i = 0; i < n; i++) {
...
function rectOutline(o: Pt[], c: Pt, u: Pt, v: Pt, per: number) {
  const p: Pt[] = [
...
function buildShape(key: ShapeKey): Pt[] {
  ...
  switch (key) {
    /* ---- Medical & Pharma: a full musculature, not a skeleton ---- */
    case "anatomy": {
      // head + neck
      ellipsoid(o, 0, -47, 0, 7.2, 9, 7.6, 120);
      tube(o, [0, -39, 0], [0, -33, 0], 3.6, 5, 40);
      // traps -> deltoids
      tube(o, [-12, -31, -1], [12, -31, -1], 5, 5, 70, 0.25);
      ellipsoid(o, -14.5, -29, 0, 5.4, 5, 5.2, 70);
      ellipsoid(o, 14.5, -29, 0, 5.4, 5, 5.2, 70);
      // ribcage barrel + lats
      tube(o, [0, -31, 0], [0, -10, 0], 11.5, 9, 190, 0.12);
      tube(o, [-11, -28, -1], [-7, -11, -1], 4, 2.6, 55);
      tube(o, [11, -28, -1], [7, -11, -1], 4, 2.6, 55);
      // pectorals
      ellipsoid(o, -5.6, -25, 5.2, 6.4, 4.4, 3, 90);
      ellipsoid(o, 5.6, -25, 5.2, 6.4, 4.4, 3, 90);
      // abdominals — four ranks of two
      for (let r = 0; r < 4; r++) {
        const y = -16 + r * 3.9;
        const w = 2.6 - r * 0.15;
        ellipsoid(o, -2.7, y, 6.1 - r * 0.25, w, 1.7, 1.4, 26);
        ellipsoid(o, 2.7, y, 6.1 - r * 0.25, w, 1.7, 1.4, 26);
      }
      // pelvis
      tube(o, [0, -9, 0], [0, -1, 0], 9.2, 8.4, 70);
      // arms out and down, as in the reference
      for (const s of [-1, 1]) {
        tube(o, [s * 14, -28, 0], [s * 24, -16, 1], 4.6, 3.4, 80, 0.4); // biceps/triceps
        tube(o, [s * 24, -16, 1], [s * 33, -3, 1], 3.3, 2.2, 70, 0.25); // forearm
        ellipsoid(o, s * 35, 1, 1, 2.3, 3.4, 1.6, 34); // hand
        // legs
        tube(o, [s * 4.6, -2, 0], [s * 6, 20, 0], 6.8, 4.6, 110, 0.35); // quad
        tube(o, [s * 6, 20, 0], [s * 6.6, 38, -0.6], 4.6, 2.6, 85, 0.45); // calf
        ellipsoid(o, s * 6.8, 42.5, 2.5, 2.6, 2.2, 4.8, 34); // foot
      }
      break;
    }
    ...
    case "camera": {
      ...
      for (const f of [[-20, 40, -10], [16, 40, -8], [bx, 40, 18]] as Pt[]) {
    ...
    case "character": {
      ...
      for (let i = 0; i < spikes; i++) {
        ...
        const base: Pt = [Math.cos(a) * 6.5, -41, Math.sin(a) * 6.5];
        const tip: Pt = [Math.cos(a) * 13 * tilt, -41 - 12 * tilt - Math.random() * 5, Math.sin(a) * 13 * tilt];
    ...
    case "graphics": {
      ...
      for (let i = 0; i < RUNGS; i++) {
        ...
        const a = t * TURNS * Math.PI * 2;
        const y = -H / 2 + t * H;
        const p1: Pt = [Math.cos(a) * R, y, Math.sin(a) * R];
        const p2: Pt = [Math.cos(a + Math.PI) * R, y, Math.sin(a + Math.PI) * R];
        seg(o, p1, p2, 16);
...
function densify(model: Pt[], target: number): Pt[] {
  ...
  while (out.length < target) {
    ...
    out.push([
      p[0] + (Math.random() - 0.5) * 0.7,
      p[1] + (Math.random() - 0.5) * 0.7,
      p[2] + (Math.random() - 0.5) * 0.7,
    ]);
  }
  return out;
}
...
const COLORS = ["#1951a8", "#36a1df", "#9fd4f5"];
const BANDS = 6; // depth buckets — lets us batch draws and still shade by depth
...
export function ShapeParticles({
  ...
  useEffect(() => {
    ...
    // Auto-fit. Each model is authored at whatever scale suited it, so measure
    // it and normalise: the widest it can ever project (at any Y rotation) is
    // its radius in XZ; vertically it's max |y|.
    let extent = 1;
    ...
    const FOCAL = 380;
    const K_MIN = FOCAL / (FOCAL + extent);
    const K_MAX = FOCAL / Math.max(1, FOCAL - extent);
    ...
    const frame = () => {
      ...
      const S = (Math.min(width, height) * 0.46) / (extent * K_MAX);
      ...
      for (let i = 0; i < COUNT; i += step) {
        ...
        // depth 0 (far) .. 1 (near), normalised to this model's own range
        const depth = Math.max(0, Math.min(0.999, (k - K_MIN) / (K_MAX - K_MIN)));
      ...
      for (let b = 0; b < BANDS; b++) {
        const t = b / (BANDS - 1);
        ctx.globalAlpha = on ? 0.3 + t * 0.68 : 0.18;
